...
#keras.applications中的预训练模型有以下几种预训练的inputshape[96*96, 128*128, 160*160, 192*192, 224*224]
#keras.applications中的预训练模型可以更改inputshape,模型结构会随之变更，但是更改inputshape后，预训练权重就不适应新的数据了，需要将整个模型重新训练。
#TF hub中的预训练模型不能更改inputshape，否则会报错。
...
filepath = "weights-applications.hdf5"
...
# 训练模型
history = model.fit(x_train,y_train,batch_size=128,epochs=6,verbose=1,callbacks=[checkpoint],validation_split=0.1)

#减少学习率再次训练
model.compile(loss='categorical_crossentropy',optimizer=tf.keras.optimizers.SGD(learning_rate=0.01),metrics=['accuracy'])
history =model.fit(x_train,y_train,batch_size=128,epochs=7,verbose=1,callbacks=[reduce_lr],validation_split=0.1)
